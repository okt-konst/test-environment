...
main(int argc, char *argv[])
{
    ...
    CHECK_RC(cfg_set_instance_fmt(CFG_VAL(STRING, "0xdead"),
             "/agent:%s/ta_conf_demo:/hex_val:", pco_iut->ta));
    ...
    CHECK_RC(cfg_get_instance_fmt(&vt, &str,
             "/agent:%s/ta_conf_demo:/hex_val:", pco_iut->ta));
    if (strcmp(str, "0xdead") != 0)
        TEST_VERDICT("hex_val reads back '%s'", str);
    free(str); str = NULL;
    ...
    TEST_STEP("Base-0 unsigned leaf (TA_CONF_TYPE_UINT0): hex, octal "
              "and decimal input are all accepted on set, a negative "
              "value is rejected");
    {
        te_errno rc;

        /*
         * A plain (non-local) set's local echo in the Configurator
         * cache is the value just written, not what the agent's own
         * get would report; unlike every other leaf tested above,
         * uint0_val's get format (plain decimal) can differ from what
         * was set (hex/octal), so the echo would hide a get that
         * never actually reached the agent. Force a real refresh from
         * the agent with cfg_synchronize_fmt(), same as citem/grp
         * below.
         */
        CHECK_RC(cfg_set_instance_fmt(CFG_VAL(STRING, "0x2a"),
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        ...
        CHECK_RC(cfg_get_instance_fmt(&vt, &str,
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        if (strcmp(str, "42") != 0)
            TEST_VERDICT("uint0_val after '0x2a' reads back '%s'", str);
        free(str); str = NULL;

        CHECK_RC(cfg_set_instance_fmt(CFG_VAL(STRING, "052"),
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        CHECK_RC(cfg_synchronize_fmt(true,
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        vt = CVT_STRING;
        CHECK_RC(cfg_get_instance_fmt(&vt, &str,
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        if (strcmp(str, "42") != 0)
            TEST_VERDICT("uint0_val after '052' reads back '%s'", str);
        free(str); str = NULL;

        CHECK_RC(cfg_set_instance_fmt(CFG_VAL(STRING, "42"),
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        CHECK_RC(cfg_synchronize_fmt(true,
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        vt = CVT_STRING;
        CHECK_RC(cfg_get_instance_fmt(&vt, &str,
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta));
        if (strcmp(str, "42") != 0)
            TEST_VERDICT("uint0_val after '42' reads back '%s'", str);
        free(str); str = NULL;

        rc = cfg_set_instance_fmt(CFG_VAL(STRING, "-1"),
                 "/agent:%s/ta_conf_demo:/uint0_val:", pco_iut->ta);
        if (TE_RC_GET_ERROR(rc) != TE_EINVAL)
        {
            TEST_VERDICT("Unexpected return status for uint0_val "
                         "set to '-1': %r", rc);
        }
    }
    ...
    TEST_STEP("List-only fixed instances (TA_CONF_LIST) and instance "
              "lookup at depth via a RO_UINT child");
    {
        unsigned int n_inst = 0;
        cfg_handle *insts = NULL;
    ...
    TEST_STEP("Read-only signed collection (TA_CONF_RO_COLL_INT)");
    {
        static const struct {
            const char *name;
            int32_t     value;
