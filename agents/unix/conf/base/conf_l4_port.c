 ...
 */
static te_errno
l4_port_alloc_property_set(int *property, uintmax_t value)
{
    int property_value;

    /* Same bound te_strtoi(..., 0, ...) enforced on the parsed value */
    if (value > INT_MAX)
...
static te_errno
l4_port_alloc_family_get(ta_conf_ctx *ctx, uintmax_t *val)
{
    UNUSED(ctx);
    *val = socket_family;
    return 0;
...
static te_errno
l4_port_alloc_family_set(ta_conf_ctx *ctx, uintmax_t val)
{
    UNUSED(ctx);
...
static te_errno
l4_port_alloc_type_get(ta_conf_ctx *ctx, uintmax_t *val)
{
    UNUSED(ctx);
    *val = socket_type;
    return 0;
...
static te_errno
l4_port_alloc_type_set(ta_conf_ctx *ctx, uintmax_t val)
{
    UNUSED(ctx);
...
static te_errno
l4_port_alloc_next_get(ta_conf_ctx *ctx, intmax_t *val)
{
...
static const ta_conf_node *const node_port =
    TA_CONF_NA("l4_port",
        TA_CONF_NA("alloc",
            TA_CONF_RO_INT("next", l4_port_alloc_next_get,
                TA_CONF_RW_UINT0("family", l4_port_alloc_family_get,
                                 l4_port_alloc_family_set),
                TA_CONF_RW_UINT0("type", l4_port_alloc_type_get,
