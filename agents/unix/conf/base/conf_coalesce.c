 ...
 */
static te_errno
coalesce_param_get(ta_conf_ctx *ctx, uintmax_t *val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
                 ...
                 uintmax_t val)
{
    ...
    if (val > UINT_MAX)
    {
        ERROR("%s(): too big value '%ju' for '%s'", __FUNCTION__, val,
              param_name);
 ...
 */
static te_errno
coalesce_param_set(ta_conf_ctx *ctx, uintmax_t val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
    const char *param_name = ta_conf_ctx_inst(ctx, "param");
    struct ethtool_coalesce *eptr;
...
static const ta_conf_node *const node_net_if_coalesce =
    TA_CONF_NA("coalesce",
        TA_CONF_NA_COMMIT("global", if_coalesce_commit,
            TA_CONF_RW_COLL_UINT("param", coalesce_param_get,
                                 coalesce_param_set,
                                 coalesce_param_list)));
 ...
 */
static te_errno
queue_param_get(ta_conf_ctx *ctx, uintmax_t *val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
 ...
 */
static te_errno
queue_param_set(ta_conf_ctx *ctx, uintmax_t val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
    const char *queue_name = ta_conf_ctx_inst(ctx, "queue");
    const char *param_name = ta_conf_ctx_inst(ctx, "param");
...
static const ta_conf_node *const node_queues =
    TA_CONF_NA_COMMIT("queues", queues_commit,
        TA_CONF_LIST("queue", queue_list,
            TA_CONF_RW_COLL_UINT("param", queue_param_get,
                                 queue_param_set,
                                 coalesce_param_list)));
