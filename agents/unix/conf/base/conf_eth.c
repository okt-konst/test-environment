...
static te_errno
eth_msglvl_get(ta_conf_ctx *ctx, uintmax_t *val)
{
    const char *ifname = ta_conf_ctx_inst(ctx, "interface");
...
static te_errno
eth_ring_tx_max_get(ta_conf_ctx *ctx, intmax_t *val)
{
    return eth_ring_size_get(ctx, false, true, val);
}

static te_errno
eth_ring_rx_max_get(ta_conf_ctx *ctx, intmax_t *val)
{
    return eth_ring_size_get(ctx, true, true, val);
}

static te_errno
eth_ring_tx_current_get(ta_conf_ctx *ctx, intmax_t *val)
{
    return eth_ring_size_get(ctx, false, false, val);
}

static te_errno
eth_ring_rx_current_get(ta_conf_ctx *ctx, intmax_t *val)
{
...
static te_errno
eth_msglvl_set(ta_conf_ctx *ctx, uintmax_t val)
{
    const char *ifname = ta_conf_ctx_inst(ctx, "interface");
    struct ethtool_value eval;
    te_errno rc;
...
static te_errno
eth_ring_size_set(ta_conf_ctx *ctx, bool is_rx, intmax_t val)
{
    const char               *ifname = ta_conf_ctx_inst(ctx, "interface");
    struct eth_if_context    *if_context;
    struct ethtool_ringparam  ethtool_ringparam = { .cmd = ETHTOOL_GRINGPARAM };
    ...
    if (val < 0)
    {
        ERROR("%s(): invalid value '%jd'", __FUNCTION__, val);
        return TE_RC(TE_TA_UNIX, TE_EINVAL);
    }
    ...
    if (is_rx)
    {
        if ((uintmax_t)val > ethtool_ringparam.rx_max_pending)
            return TE_RC(TE_TA_UNIX, TE_ERANGE);
        ethtool_ringparam.rx_pending = val;
    }
    else
    {
        if ((uintmax_t)val > ethtool_ringparam.tx_max_pending)
            return TE_RC(TE_TA_UNIX, TE_ERANGE);
...
static te_errno
eth_ring_rx_current_set(ta_conf_ctx *ctx, intmax_t val)
{
    return eth_ring_size_set(ctx, true, val);
}

static te_errno
eth_ring_tx_current_set(ta_conf_ctx *ctx, intmax_t val)
{
...
static te_errno
eth_channels_get(ta_conf_ctx *ctx, intmax_t *val)
{
    ...
    *val = (vp != NULL) ? (intmax_t)*vp : -1;

    return 0;
}

static te_errno
eth_channels_set(ta_conf_ctx *ctx, intmax_t val)
{
    const char                *iface = ta_conf_ctx_inst(ctx, "interface");
    struct ethtool_channels    channels = { .cmd = ETHTOOL_GCHANNELS };
    cfg_oid                   *oid_parsed;
    ...
    if (val > UINT32_MAX)
    {
        ERROR("%s(): too big value '%jd'", __FUNCTION__, val);
        return TE_RC(TE_TA_UNIX, TE_ERANGE);
    }
...
static const ta_conf_node *const node_feature =
    TA_CONF_NODE((.name = "feature", .type = TA_CONF_TYPE_BOOL,
...
static const ta_conf_node *const node_ring =
    TA_CONF_NA("ring",
        TA_CONF_NA("rx",
            TA_CONF_RW_INT("current", eth_ring_rx_current_get,
                           eth_ring_rx_current_set),
            TA_CONF_RO_INT("max", eth_ring_rx_max_get)),
        TA_CONF_NA("tx",
            TA_CONF_RW_INT("current", eth_ring_tx_current_get,
                           eth_ring_tx_current_set),
            TA_CONF_RO_INT("max", eth_ring_tx_max_get)));

static const ta_conf_node *const node_channels =
    TA_CONF_NA("channels",
        TA_CONF_NA("combined",
            TA_CONF_RW_INT("current", eth_channels_get, eth_channels_set),
            TA_CONF_RO_INT("maximum", eth_channels_get)),
        TA_CONF_NA("other",
            TA_CONF_RW_INT("current", eth_channels_get, eth_channels_set),
            TA_CONF_RO_INT("maximum", eth_channels_get)),
        TA_CONF_NA("rx",
...
static const ta_conf_node *const node_msglvl =
    TA_CONF_RW_UINT0("msglvl", eth_msglvl_get, eth_msglvl_set);
