...
static te_errno
phy_speed_get_common(ta_conf_ctx *ctx, bool admin, intmax_t *val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
    unsigned int speed;
    te_errno rc;
     ...
     */
    *val = (speed == (unsigned int)SPEED_UNKNOWN) ? -1 : (intmax_t)(int)speed;
    return 0;
}

/* Get value of agent/interface/phy/speed_oper */
static te_errno
phy_speed_oper_get(ta_conf_ctx *ctx, intmax_t *val)
{
 ...
 */
static te_errno
phy_speed_admin_get(ta_conf_ctx *ctx, intmax_t *val)
{
...
static te_errno
phy_speed_admin_set(ta_conf_ctx *ctx, intmax_t val)
{
    const char *if_name = ta_conf_ctx_inst(ctx, "interface");
 ...
 */
static te_errno
phy_state_get(ta_conf_ctx *ctx, uintmax_t *val)
{
...
static const ta_conf_node *const node_phy =
    TA_CONF_NA_COMMIT("phy", phy_commit,
        ...
        TA_CONF_RO_INT("speed_oper", phy_speed_oper_get),
        TA_CONF_RW_INT("speed_admin", phy_speed_admin_get,
                       phy_speed_admin_set),
        ...
        TA_CONF_RO_UINT("state", phy_state_get));
