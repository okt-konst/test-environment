...
codec_get(ta_conf_ctx *ctx, char *value)
{
    ...
    switch (n->type)
    {
        ...
        case TA_CONF_TYPE_INT:
        {
            intmax_t v;
            te_errno rc = n->get.as_int(ctx, &v);

            if (rc != 0)
                return rc;
            snprintf(value, RCF_MAX_VAL, "%jd", v);
            return 0;
        }

        case TA_CONF_TYPE_UINT:
        case TA_CONF_TYPE_UINT0:
        {
            uintmax_t v;
            te_errno rc = n->get.as_uint(ctx, &v);

            if (rc != 0)
                return rc;
            snprintf(value, RCF_MAX_VAL, "%ju", v);
            return 0;
        }

        case TA_CONF_TYPE_HEX32:
        {
            uintmax_t v;
            te_errno rc = n->get.as_uint(ctx, &v);
            ...
            if (rc != 0)
                return rc;
            snprintf(value, RCF_MAX_VAL, "0x%jx", v);
            return 0;
        }

        case TA_CONF_TYPE_BOOL:
        {
            bool v;
        ...
        case TA_CONF_TYPE_ENUM:
        {
            int v;
            te_errno rc = n->get.as_enum(ctx, &v);
            const char *s;

            if (rc != 0)
                return rc;

            s = te_enum_map_from_any_value(n->map, v, NULL);
            if (s == NULL)
            {
            ...
            TE_STRLCPY(value, s, RCF_MAX_VAL);
            return 0;
        }

        case TA_CONF_TYPE_NONE:
        default:
 ...
 * range instead of rejecting it.  TA_CONF_TYPE_UINT, TA_CONF_TYPE_UINT0
 * and TA_CONF_TYPE_HEX32 values must never be parsed that way, so
 * callers check for a leading '-' (after skipping optional whitespace,
 * as strtoumax() itself would) before invoking te_strtoumax().
 */
static te_errno
...
codec_set(ta_conf_ctx *ctx, const char *value)
{
    ...
    switch (n->type)
    {
        ...
        {
            intmax_t v;
            te_errno rc = te_strtoimax(value, 10, &v);

            if (rc != 0)
                return TE_RC(TE_RCF_PCH, rc);
            return n->set.as_int(ctx, v);
        }
        ...
        case TA_CONF_TYPE_UINT:
        {
            ...
            if (rc != 0)
                return rc;
            rc = te_strtoumax(value, 10, &v);
            if (rc != 0)
                return TE_RC(TE_RCF_PCH, rc);
            return n->set.as_uint(ctx, v);
        }
        ...
        case TA_CONF_TYPE_HEX32:
        {
            ...
            if (rc != 0)
                return rc;
            rc = te_strtoumax(value, 16, &v);
            if (rc != 0)
                return TE_RC(TE_RCF_PCH, rc);
            /* TA_CONF_TYPE_HEX32 is 32-bit by contract */
            if (v > UINT32_MAX)
        ...
        {
            uintmax_t v;
            te_errno rc = te_strtoumax(value, 10, &v);

            if (rc != 0)
                return TE_RC(TE_RCF_PCH, rc);
            if (v > 1)
                return TE_RC(TE_RCF_PCH, TE_EINVAL);
            return n->set.as_bool(ctx, v != 0);
        }

        case TA_CONF_TYPE_ENUM:
        {
            int v = te_enum_map_from_str(n->map, value, INT_MIN);
...
validate(const ta_conf_node *n)
{
    ...
    if (n->type == TA_CONF_TYPE_ENUM)
    {
        if (n->map == NULL)
        {
            ERROR("node '%s': ENUM node without a map", n->name);
            return TE_RC(TE_RCF_PCH, TE_EINVAL);
        }
    }
    else if (n->map != NULL)
    {
        ERROR("node '%s': map set on a non-ENUM node", n->name);
        return TE_RC(TE_RCF_PCH, TE_EINVAL);
    }
