...
turn be overridden by an ``<objective>`` element in ``package.xml``.
That objective (and the objective of an enclosing session or package,
and of an argument value) can reference the arguments and variables of
the iteration; see :ref:`Variable expansion in objectives
